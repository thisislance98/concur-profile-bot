...
Concur profile information through natural language requests using the Concur Profile SDK.
"""
...
def initialize_sdk():
    ...
    try:
        ...
        print(f"SDK initialized successfully. Authenticated as: {profile.first_name} {profile.last_name}")
        return True
...
# System prompt updated for SDK usage
SYSTEM_PROMPT = """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Concur Profile Bot powered by Claude + SDK')
    ...
    # get-profile command
    get_profile_parser = subparsers.add_parser('get-profile', help='Get profile information')
    get_profile_parser.add_argument('--login-id', help='Login ID of user to get profile for')
    ...
    if not all([CONCUR_CLIENT_ID, CONCUR_CLIENT_SECRET, CONCUR_USERNAME, CONCUR_PASSWORD]):
        ...
        print("Please ensure all required Concur API credentials are set")
        sys.exit(1)
    
    # Initialize the SDK
    if not initialize_sdk():
        print("Failed to initialize Concur Profile SDK")
        sys.exit(1)
    ...
    if args.command == 'get-profile':
        try:
            if args.login_id:
                profile = sdk.get_profile_by_login_id(args.login_id)
            else:
                profile = sdk.get_current_user_profile()
            
            print(f"Profile for: {profile.first_name} {profile.last_name}")
